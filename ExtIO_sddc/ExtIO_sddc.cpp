...
int64_t EXTIO_API StartHW64(int64_t LOfreq)
{
	...
	if (RadioHandler.IsReady()) //  HF103 connected
	{
		...
		hb = fw >> 8;
		lb = (uint8_t) fw;
		sprintf(ebuffer, "%s v%0.2f  |  FX3 v%d.%02d  |  %s ",SWNAME, VERSION ,hb,lb, radioname[global.radio] );
...
int EXTIO_API GetAttenuators(int atten_idx, float* attenuation)
{
    ...
    EnterFunction();
	if ((RadioHandler.GetmodeRF() == VHFMODE) && (global.radio != HF103 ))
...
int  EXTIO_API ExtIoGetSetting(int idx, char * description, char * value)
{
	...
	case 1:	snprintf(description, 1024, "%s", "RadioHWtype");	snprintf(value, 1024, "%d", global.radio);			return 0;
...
void EXTIO_API ExtIoSetSetting(int idx, const char * value)
{
	...
	case 1:
		...
		{
			global.radio = (radiotype)(tempInt);
		}
	case 2:
